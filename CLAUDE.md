...
Hazard-Adjusted Broadband Reliability Index — North Carolina (primary) + Tennessee (comparison).
...
| `scripts/build_habri_tn.py` | Full HABRI pipeline for Tennessee (95 counties, 1,701 tracts) | READY — Apr 2026 |
| `scripts/compare_helene_nc_tn.py` | Cross-state WNC vs. Eastern TN Helene comparison figures | READY — Apr 2026 |
...
**FCC BDC adaptive weighting (Apr 2026 — PENDING BDC DOWNLOAD):**
Adaptive per-tract I_F weights implemented. Road/tower weights interpolate between
wired-endpoint (road=0.40, tower=0.15) and wireless-endpoint (road=0.15, tower=0.40)
based on each tract's p_wired from FCC BDC availability data. Latency and power_grid
weights are constant (0.25 and 0.20). Uniform weights remain default until BDC data
is downloaded. To activate:
...
data/processed/infra_tn_fragility.gpkg            # TN I_F sub-index
...
**Baseline results (Apr 2026 — 4-component I_F including power grid):**
...
incomplete; pending Tennessee pipeline completion and cross-state comparison results).

**The NC index is final.** All NC gates resolved as of Apr 2026:

1. Q3 2024 – Q4 2025 time series complete; figures in `data/processed/habri_timeseries_*.png`
2. Power grid integrated (4-component I_F); baseline and all quarterly outputs regenerated
3. Tennessee extension: `scripts/build_habri_tn.py` and `scripts/compare_helene_nc_tn.py` ready
4. Manuscript draft pending: Tennessee pipeline run, cross-state comparison results, results/discussion rewrite
