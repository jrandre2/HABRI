...
Infrastructure Fragility (35% of score) measures the physical vulnerability of the communications network through four indicators: cell tower scarcity (25%), broadband latency (25%), road network bottlenecks (30%), and power grid exposure (20%). The road component uses betweenness centrality to identify tracts whose connectivity depends on a small number of critical road segments. The power grid component measures electric transmission line density — tracts with sparse high-voltage infrastructure face longer distribution runs that are more vulnerable to storm damage, and losing grid power directly causes broadband outages. Tower and road weights are further adjusted per tract based on FCC Broadband Data Collection data: tracts served primarily by wired providers receive a higher road weight, while wireless-dominant tracts receive a higher tower weight.
...
- The road network proxy was independently validated by comparing fixed versus mobile broadband degradation during Helene. Road fragility predicted fixed latency degradation (rho = -0.287, p < 0.001) but not mobile/cellular degradation (rho = +0.078, not significant), confirming that road right-of-way co-location drives the signal rather than simple geography.
